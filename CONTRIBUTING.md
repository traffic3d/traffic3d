...
The constructor can have as many or as little parameters as needed but the `super().__init__` function needs to have the `port` of the socket (normally `13000`) and the `image path` of the screenshots that are generated by Unity (normally `../Traffic3D/Assets/Screenshots`).

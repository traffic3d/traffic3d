   ...
   // public List<Transform> MaterialChange;
    ...
    void Start()
    {
     ...
     //   m = TrafficLight.GetComponent<MaterialChange>();
		...
		j = trafficLight.GetComponent<TLaction3>();

	

		startTime = Time.time;

		path1 = GameObject.Find("mypathy1").GetComponent<Transform>();  //mypathx
		path2 = GameObject.Find("mypathx1").GetComponent<Transform>();
	...
	{
		if (j.CM.color.Equals(material2.color)) {
			print ("red2");
		}
	}*/
...
des = true;
	...
	{
		if (currentNode == nodes.Count - 3 && j.CM.color.Equals(material2.color)) 
	...
	{
		if (currentNode == nodes.Count - 3 && j.CM.color.Equals(material4.color)) 

	...
	public class AssetCollector
	{
		...
		List<string> StripTargetPathsAll (bool isUseCodeStrip, string[] pathes)
		{
			var files = pathes.SelectMany (c => Directory.GetFiles (c, "*.*", SearchOption.AllDirectories))
					...
					.Where (item => System.IO.Path.GetExtension (item) != ".js")
					.Where (item => System.IO.Path.GetExtension (item) != ".dll")
			...
			if (isUseCodeStrip == false) {
				files = files.Where (item => System.IO.Path.GetExtension (item) != ".cs");
			}
		...
		{
			var codePaths = deleteFileList.Where (fileName => System.IO.Path.GetExtension (fileName) == ".cs");
		...
		void UnregistEditorCodes ()
		{
			...
			var editorcodes = Directory.GetFiles ("Assets", "*.*", SearchOption.AllDirectories)
				.Where (fileName => System.IO.Path.GetExtension (fileName) == ".cs")
